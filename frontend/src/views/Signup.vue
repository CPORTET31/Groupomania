  ...
  <main class="main main--connect">
    <form class="w-75 align-items-center form-block d-flex m-auto shadow rounded">
      <div class="form-block--left d-flex flex-column justify-content-center block-demi-container p-3 text-right align-self-stretch">
        <img class="logo align-self" src="../assets/icon-left-font-monochrome-black.svg" alt="Logo Groupomania" />
        <p>
          <small>
      ...
      if (
        ...
        axios
          ...
          .then(response => {
            ...
            axios.post("http://localhost:3000/api/user/login", this.dataSignup)
              .then(response => {
                localStorage.setItem('token',response.data.token);
                this.$store.commit("setAuthentication", true);
                this.$router.replace({ name: "Wall" });
              })
          ...
          .catch(
            error => {
              if(String(error).includes("409")) {
                alert("Echec ! Ce nom d'utilisateur existe déja.")   
              } else {
                alert("Echec ! Une erreur inconnu s'est produite, veuillez contacter l'administrateur.")
              }
            }
          );
      } else {
        alert("Echec ! Un problème est survenue avec vos saisies.\nVotre mot de passe doit contenir au minimum 8 caractères, dont une majuscule, une minuscule, un chiffre et un caractère spécial).");
      }
    }
  }
};
